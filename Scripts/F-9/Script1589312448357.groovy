...
for (def row = 1; row <= findTestData('InValidPassConfirmation').getRowNumbers(); row++) {
    ...
    WebUI.verifyElementVisible(findTestObject('Rejestracja Podane hasla rozna sie'))

    WebUI.getText(findTestObject('Rejestracja Podane hasla rozna sie'))

    WebUI.verifyMatch('PODANE HASŁA RÓŻNIĄ SIĘ!', 'PODANE HASŁA RÓŻNIĄ SIĘ!', true)
}
